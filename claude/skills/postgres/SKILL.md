---
name: postgres
description: Expert guidance for PostgreSQL design decisions — indexes, queries, schema, and performance. Invoke when writing migrations, adding indexes, designing schemas, optimizing queries, or reviewing database-related changes. Uses progressive disclosure by referencing topic-specific files on-demand.
---
...
Use the Read tool to access files in these situations:
...
Identify what you'll be working on (indexes, schema, queries, etc.)
  ↓
Read relevant reference file using the Read tool
  ↓
